// ==========================================
// TradingView To N8N — Content Script for TradingView
...
async function handleTimeframeChange(tf) {
  ...
  console.log(`[tvn8n] Changing TimeFrame to: ${targetTf}`);

  let result = null;

  // Try Strategy 1: Direct Toolbar Button
  if (tryDirectToolbarClick(targetTf)) {
    console.log(`[tvn8n] TimeFrame changed via direct toolbar button: ${targetTf}`);
    result = { success: true, method: 'toolbar' };
  } else if (await tryDropdownMenuClick(targetTf)) {
    // Try Strategy 2: Dropdown Menu
    console.log(`[tvn8n] TimeFrame changed via dropdown menu: ${targetTf}`);
    result = { success: true, method: 'dropdown' };
  } else {
    // Strategy 3: Keyboard Simulation
    console.log(`[tvn8n] Fallback to keyboard simulation for: ${targetTf}`);
...
console.log('[tvn8n] Content script loaded on TradingView');
