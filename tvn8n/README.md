# TradingView To N8N — Chrome Extension

**TradingView To N8N** เป็น Chrome Extension (Manifest V3) สำหรับจับภาพหน้าจอ กราฟหุ้น/คริปโต จาก **TradingView** และส่งไปยัง **N8N Workflow** อัตโนมัติผ่าน **N8N Webhook** ตามช่วงเวลาและ Timeframe ที่กำหนด
...
- **รองรับทั้ง Production Webhook & Test Webhook**: กำหนด URL แยกกันระหว่างโหมดการทำงานจริงและโหมดทดสอบ
...
- **จัดการ Rate Limit อัตโนมัติ (HTTP 429 Retry)**: หากชนข้อจำกัดการส่งภาพ ระบบจะหน่วงเวลาแล้วลองส่งใหม่อัตโนมัติตามค่า `retry_after`
...
├── background.js     # Background Service Worker (จัดการ Alarms, Screenshot & N8N Webhook)
...
1. **ดาวน์โหลด หรือ Clone โปรเจกต์**:
   ```bash
   git clone https://github.com/your-username/tvn8n.git
   ```
...
5. เลือกโฟลเดอร์โปรเจกต์ `tvn8n`
6. ส่วนขยาย **TradingView To N8N** จะปรากฏขึ้นใน Chrome พร้อมใช้งาน!
...
- **N8N Webhook URL (Production)**: URL ของ N8N Webhook Node สำหรับการทำงานจริง (เช่น `https://fightable-unprejudicial-deonna.ngrok-free.dev/webhook/tv`)
- **N8N Webhook URL (Test Mode)**: URL สำหรับการทดสอบใน N8N Editor (เช่น `https://fightable-unprejudicial-deonna.ngrok-free.dev/webhook-test/tv`)
- คลิก **บันทึก (Save)** เพื่อจัดเก็บข้อมูลลงใน `chrome.storage.local`

### 2. ปุ่มควบคุม (Control Buttons)
- **🧪 Test (ทดสอบ)**: ส่งภาพกราฟตาม **TF1** ไปยัง N8N Test Webhook ทันที 1 ภาพ เพื่อทดสอบว่า Webhook Node และ Workflow ใน N8N รับข้อมูลได้ถูกต้อง
- **▶️ Play (เริ่มทำงาน)**: เริ่มต้นระบบการส่งภาพอัตโนมัติไปยัง N8N Production Webhook โดยจะทำการส่งรอบแรกทันที และตั้งเวลาส่งในรอบถัดไปตามที่กำหนด
...
sequenceDiagram
    ...
    UI->>BG: ส่งคำสั่ง 'start' หรือ 'test' พร้อม N8N Webhook URLs
    ...
    loop ทุกๆ X นาที (Alarm Fired)
        ...
        loop แต่ละ Timeframe (TF1...TFn)
            ...
            BG->>N8N: ส่งรูปภาพ PNG + Timeframe Metadata ไปยัง N8N Webhook
...
1. **`background.js` (Service Worker)**:
   ...
   - โฟกัสแท็บ TradingView, สั่งรีเฟรชหน้าก่อนจับภาพ, สั่งงาน `content.js`, จับภาพหน้าจอด้วย `captureVisibleTab` และส่ง HTTP POST (FormData) ไปยัง N8N Webhook Node
   - แนบไฟล์ภาพ (`file`), ชื่อ Timeframe (`timeframe`), และ Timestamp (`timestamp`) ไปใน FormData เพื่อให้ N8N ดึงข้อมูลไปใช้งานได้ทันที
...
ส่วนขยาย **TradingView To N8N** ได้รับการปรับแต่งให้รองรับการทำงานบน **Windows 7** อย่างสมบูรณ์ 100%:
