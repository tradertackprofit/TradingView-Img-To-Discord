// ==========================================
// TradingView To N8N — Side Panel UI Logic
// ==========================================
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btnPlay.addEventListener('click', async () => {
    if (isRunning) {
      ...
      if (!webhook) {
        showToast('กรุณาใส่ N8N Webhook URL (Production) ก่อน', 'error');
        webhookUrl.focus();
        return;
      }
      if (!isValidUrl(webhook)) {
        showToast('Webhook URL ไม่ถูกต้อง (ต้องขึ้นต้นด้วย http:// หรือ https://)', 'error');
        webhookUrl.focus();
        return;
      }
  ...
  btnTest.addEventListener('click', async () => {
    const targetWebhook = (testWebhookUrl && testWebhookUrl.value.trim()) || webhookUrl.value.trim();
    if (!targetWebhook) {
      showToast('กรุณาใส่ N8N Webhook URL ก่อน', 'error');
      if (testWebhookUrl) testWebhookUrl.focus(); else webhookUrl.focus();
      return;
    }
    if (!isValidUrl(targetWebhook)) {
    ...
    addLog('🧪 ทดสอบส่งรูป TF1 ไปยัง N8N...', 'info');
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    if (areaName === 'local') {
      if (changes.tvn8nState || changes.tv2disState) {
        const newState = (changes.tvn8nState || changes.tv2disState).newValue;
        if (newState) {
          setRunningState(newState.isRunning);
        }
      }
      if (changes.tvn8nSettings || changes.tv2disSettings) {
        loadSettings();
      }
    }
  });
  ...
  async function saveSettingsToStorage() {
    ...
    const settings = {
      ...
      testWebhookUrl: testWebhookUrl ? testWebhookUrl.value.trim() : '',
      timeframes: []
    };
    ...
    await chrome.storage.local.set({ tvn8nSettings: settings, tv2disSettings: settings });
  }

  function loadSettings() {
    chrome.storage.local.get(['tvn8nSettings', 'tv2disSettings'], (result) => {
      const settings = result.tvn8nSettings || result.tv2disSettings;
      if (!settings) {
        webhookUrl.value = 'https://fightable-unprejudicial-deonna.ngrok-free.dev/webhook/tv';
        if (testWebhookUrl) {
          testWebhookUrl.value = 'https://fightable-unprejudicial-deonna.ngrok-free.dev/webhook-test/tv';
        }
      ...
      webhookUrl.value = settings.webhookUrl || 'https://fightable-unprejudicial-deonna.ngrok-free.dev/webhook/tv';
      if (testWebhookUrl) {
        testWebhookUrl.value = settings.testWebhookUrl || 'https://fightable-unprejudicial-deonna.ngrok-free.dev/webhook-test/tv';
      }
  ...
  function checkRunningState() {
    chrome.storage.local.get(['tvn8nState', 'tv2disState'], (result) => {
      const state = result.tvn8nState || result.tv2disState;
  ...
  function startCountdown() {
    ...
    const updateTimerText = () => {
      chrome.storage.local.get(['tvn8nState', 'tv2disState', 'tvn8nSettings', 'tv2disSettings'], (result) => {
        const state = result.tvn8nState || result.tv2disState;
        const settings = result.tvn8nSettings || result.tv2disSettings || {};
